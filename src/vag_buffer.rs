...
// the repr(u8) attribute, each enum variant is stored as a u8
#[repr(u8)]
pub enum Color {
    Black = 0,
    Blue = 1,
...
// we need the repr(C) attribute. It guarantees that the struct’s fields
...
// Writer always writes to the bottom row of the buffer,
// so we don't need to keep track of the row position.
pub struct Writer {
